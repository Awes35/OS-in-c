...
HIDDEN pcb_PTR pcbFree_h; /* ptr to head of the pcbFree list */
...
pcb_PTR allocPcb(){
	...
	pcb_PTR temp1 = pcbFree_h; /* element that we will remove from pcbFreeList */
...
void initPcbs(){
	int i;
	static pcb_t procTable[MAXPROC]; /* array of pcb's */
	pcbFree_h = NULL;
	for(i = 0; i < MAXPROC; i++){
...
void insertProcQ(pcb_PTR *tp, pcb_PTR p){
	...
	/* else process queue not empty */
		/* inserting p at the tail, and it becomes the new tail. This is a circular queue, 
		so the tail must point to head, ie: (*tp)->next must always be the front of the queue */
		p->p_next = (*tp)->p_next;
		p->p_prev = *tp;
		((*tp)->p_next)->p_prev = p;
		(*tp)->p_next = p;
...
pcb_PTR outChild(pcb_PTR p){
	...
	/* there's only one pcb in the stack */
		if (current == p){ 
		/* if the pcb p points to is the only pcb in the stack & we want to remove it */
			/* removing p: */
			pcb_t *temp1 = current;
			temp1->p_prnt = NULL;
			p->p_child = NULL;
	...
	/* The stack has at least 2 pcbs in it */
