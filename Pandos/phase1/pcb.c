...
pcb_PTR allocPcb(){
	...
	if (temp1->p_next == NULL){ /* there's only one element in pcbFreeList */
		pcbFree_h = NULL;
	}
...
void insertProcQ(pcb_PTR *tp, pcb_PTR p){
    if (emptyProcQ(*tp)){ /* if the process queue is empty */
    	...
    	*tp = p;
        ...
        /* inserting p at the tail, and it becomes the new tail.
...
pcb_PTR outChild(pcb_PTR p){
	...
	/* At this point, we know the stack has at least two pcbs in it. */
	if (p->p_next_sib == NULL){ /* if the pcb p points to is the last pcb in the stack */
		(p->p_prev_sib)->p_next_sib = NULL; /* removing p */
		p->p_prev_sib = NULL;
		p->p_prnt = NULL;
		return p;
	}
	/* At this point, we know the pcb p points to is not the last pcb in the stack */
	if (p->p_prev_sib == NULL){ /* if the pcb p points to is the first pcb in the stack */
		(p->p_next_sib)->p_prev_sib = NULL; /* removing p */
		(p->p_prnt)->p_child = p->p_next_sib;
		p->p_prnt = NULL;
		p->p_next_sib = NULL;
		return p;
	}
	/* If we reach this point in the function, then the pcb p points to is in the middle of the stack */
	(p->p_prev_sib)->p_next_sib = p->p_next_sib; /* removing p */
