...
void p5gen() {
	unsigned int exeCode = pFiveSupport.sup_exceptionState[GENERALEXCEPT].s_cause;
	exeCode = (exeCode & CAUSEMASK) >> 2;
	switch (exeCode) {
	case BUSERROR:
		print("Bus Error: Access non-existent memory\n");
		pFiveSupport.sup_exceptionState[GENERALEXCEPT].s_pc = (memaddr)p5a;   /* Continue with p5a() */
		pFiveSupport.sup_exceptionState[GENERALEXCEPT].s_t9 = (memaddr)p5a;   /* Continue with p5a() */
		break;
		
	case RESVINSTR:
		print("privileged instruction\n");
		/* return in kernel mode */
		pFiveSupport.sup_exceptionState[GENERALEXCEPT].s_pc = (memaddr)p5b;   /* Continue with p5b() */
		pFiveSupport.sup_exceptionState[GENERALEXCEPT].s_t9 = (memaddr)p5b;   /* Continue with p5b() */
		pFiveSupport.sup_exceptionState[GENERALEXCEPT].s_status = pFiveSupport.sup_exceptionState[GENERALEXCEPT].s_status & KUPBITOFF;
		break;
	...
	LDST(&(pFiveSupport.sup_exceptionState[GENERALEXCEPT]));
}
...
void p5mm() {
	...
	pFiveSupport.sup_exceptionState[PGFAULTEXCEPT].s_status = pFiveSupport.sup_exceptionState[PGFAULTEXCEPT].s_status | KUPBITON;	/* user mode on 	*/
	pFiveSupport.sup_exceptionState[PGFAULTEXCEPT].s_pc = (memaddr)p5b;   						/* return to p5b()	*/
	pFiveSupport.sup_exceptionState[PGFAULTEXCEPT].s_t9 = (memaddr)p5b;						/* return to p5b()	*/

	LDST(&(pFiveSupport.sup_exceptionState[PGFAULTEXCEPT]));
}

/* p5's SYS trap handler */
void p5sys() {
	unsigned int p5status = pFiveSupport.sup_exceptionState[GENERALEXCEPT].s_status;
	...
	pFiveSupport.sup_exceptionState[GENERALEXCEPT].s_pc = pFiveSupport.sup_exceptionState[GENERALEXCEPT].s_pc + 4;   /*	 to avoid SYS looping */
	LDST(&(pFiveSupport.sup_exceptionState[GENERALEXCEPT]));
}
